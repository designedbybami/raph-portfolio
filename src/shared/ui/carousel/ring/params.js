...
export const WEIGHTS = { Light: 300, Regular: 400, Medium: 500, Semibold: 600, Bold: 700 };

/**
 * Every tunable in one place, a fresh object per mount so the dev panel cannot
 * leak edits between mounts. Every px figure is quoted at `refWidth` and scaled
 * from there. The `narrowAt` / `tightAt` bands re-proportion the ring rather
 * than just scaling it; they multiply the fit and they stack.
 */
export function defaultParams(projectCount) {
  return {
    // -- fit ------------------------------------------------------------ 14" MacBook Pro at default scaling: 3024x1964 native, 1512x982 points.
    refWidth: 1512,
    ...
    planeSize: 90, // long edge in px; aspect locked at 4 : 5 portrait
    count: projectCount, // one plane per project, so the deal comes out even
    ringRadius: 340,
    ...
    // -- loading ---------------------------------------------------------
    // The entry launches once the seed is formed and wearing its own art. holdAfter is a beat after that and wants to stay near zero.
    holdAfter: 0,
    loaderChase: 0.18,
    loaderBottom: 1, // vh
    ...
    // -- entry timing ----------------------------------------------------
    // Paced in two halves: the seed's birth and launch stay quick (they continue the dot the nav left behind), while the spread onward is what people actually watch. Shortening that flattens the stagger into a pop.
    stagger: 0.34,
    launchTime: 0.85,
    spreadEase: "power2.out",
    spreadTime: 3.6,
    // Where the ring goes once formed. stageAt is a fraction of the spread, so 0.5 starts the move midway through the ring drawing.
    stageAt: 0.7,
    spinTurns: 1,
    spinTime: 2.6,
    ...
    // [number . name] left, [type . year] right. Insets and gaps in vw so the pairs hold their relationship as the window changes.
    metaLeft: 5.5,
    metaRight: 5.5,
    // The right lockup sits at the bottom rather than centre so it stays clear of the title column at top-right, however long that list grows.
    metaBottom: 8, // vh
    metaGapL: 4.7,
    metaGapR: 3.6,
    metaWidth: 34, // this box is the filter region
    nameSize: (36 / 1440) * 100, // vw, quoted at 1440
    // The title (this card's name). Only one weight is loaded for this font, so requesting another would just synthesize a fake bold. No lowercase glyphs either; meta.js forces uppercase wherever this is used.
    nameFont: "var(--font-display)",
    nameWeight: 400,
    idxSize: (16 / 1440) * 100, // a step lighter and smaller than the name
    // Everything else in the lockups (year, the hidden index number) and the category, which reuses this same paragraph font at a bolder weight.
    idxFont: "var(--font-montserrat)",
    idxWeight: 400,
    categoryWeight: 600,
    listSize: 1, // vw; the column's line height is unitless so rows follow
    ...
    // -- pointer ---------------------------------------------------------
    // Nothing is drawn at the cursor. It softens the field around itself, leans the nearest planes toward it and strings honey back between them.
    hover: true,
    ...
    // How the cards either side of the hovered one get out of its way. The pointer block above answers to the cursor; these answer to the card.
    sideScale: 0.035,
    sidePush: 17, // px
    sideDim: 0.15,
    ...
    tagGap: 6,
    // Offset off the cursor deliberately: sitting under it, the tag covers the thing being pointed at. World px, so +y is up like posY.
    tagX: 64,
    tagY: -38,
    tagFrost: 0.16,
    tagRim: 0.02,
