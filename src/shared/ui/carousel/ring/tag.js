...
// Shared between the canvas that rasterises the label and the uniform that tells the shader how big it is.
export const TAG_W = 104;
export const TAG_H = 40;

/**
 * The "View" tag that rides the cursor over a card. Drawn in the ring's own
 * shader pass rather than as an element over the canvas, which is what lets
...
export function createTag(params, uniforms) {
  ...
  const show = (on) => {
    ...
    if (on) {
      // Two axes on separate springs, so it wobbles as it forms rather than scaling up as one rigid piece.
  ...
  const load = (onReady) => {
    ...
    arrow.src = "/icons/arrow-right-up.svg";
  };
