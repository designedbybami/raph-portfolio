...
export function createSplitText(group, params) {
  ...
  const build = () => {
    ...
    // Above display resolution, since type shows softness first and these canvases are tiny.
    ...
    glyphs.forEach((ch, i) => {
      ...
      if (ch.trim()) {
        ...
        // Cell is the advance box plus symmetric padding, so centring on the advance keeps the run correctly spaced.
