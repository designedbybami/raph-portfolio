...
export function mountGui(GUI, { params, state, info, actions }) {
  ...
  // -- fit -----------------------------------------------------------------
  // Every px param in the folders below is quoted at the reference window. Tuning at some other size and leaving it alone is what makes the ring look right here and wrong everywhere else, so the live size sits next to it.
  ...
  text
    // Only families with an @font-face block in globals.css; anything else silently falls back to system sans and looks like a bug.
  ...
  meta.add(params, "nameWeight", WEIGHTS).name("title weight").onChange(styleMeta); // prettier-ignore
  meta.add(params, "idxWeight", WEIGHTS).name("year/number weight").onChange(styleMeta); // prettier-ignore
  meta.add(params, "categoryWeight", WEIGHTS).name("category weight").onChange(styleMeta); // prettier-ignore
  ...
  // Re-announces whatever is already at the front, so the morph can be watched without spinning to a new card each time.
  ...
  // Apart from the pointer folder above: those answer to the cursor and are always live, these answer to the card under it and only while there is one.
