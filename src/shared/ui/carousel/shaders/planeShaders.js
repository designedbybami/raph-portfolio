// Each element of every uniform array below costs a fragment uniform slot. The guaranteed WebGL2 minimum is 224 vec4s, so the link parameters are packed into one vec4 array and the image index is derived arithmetically rather than passed as a tenth array.
...
export const fragmentShader = /* glsl */ `
  ...
  // xy = 0..1 per axis, z = brightness, w = atlas cell. Packed together because GLSL ES gives every uniform array element a full vec4 row regardless, so zw were already being paid for. The cell is CPU-resolved: it depends on the plane's ring position, not its index.
  ...
  // All the artwork lives in one atlas: ESSL 1.00 cannot index an array of samplers with a varying index, so a per-plane texture is not an option.
  ...
  // --- pointer -------------------------------------------------------------
  // Nothing is drawn at the cursor; it only softens the field and sends a wake through the surface. Packed into vec4s for the uniform budget.
  ...
  // --- the cursor tag ------------------------------------------------------
  // Drawn in this pass, not as an element over the canvas, so its label can invert against the pixels beneath it and its glass refracts like the lip.
  ...
  // --- glass lip -----------------------------------------------------------
  // A band top and bottom behaving like the rounded edge of thick glass. The whole scene is evaluated from p, so warping p here refracts planes and honey together with no second pass.
  ...
  float glassBend(inout vec2 p) {
    ...
    // Circular profile: sharp only at the very edge, which reads as thickness.
    float bend = 1.0 - sqrt(max(0.0, 1.0 - t * t));

    float s = sign(p.y);
    // Sampling back toward centre throws content outward. Signed, so negatives invert the lip and compress instead.
  ...
  // The honey between two planes: a flat slab, pinched in the middle, drooping. Swept as a box rather than a capsule, whose round cross-section would bulge past the planes' flat sides at full merge.
  ...
  // By difference: a pill's normal is vertical along the edges and radial round the ends, so nothing simpler aims the refraction correctly throughout.
  ...
  // Smooth minimum, what makes the shapes read as liquid. Also behaves as a plain min() when a is the 1e6 sentinel.
  ...
  void main() {
    // Unbent: the tag is pinned to the cursor, not to the lip's warped space.
    vec2 ps = (vUv - 0.5) * uResolution;

    vec2 p = ps;
    float bend = glassBend(p);

    // Warp before the field is read, same order as the lip. Flat through the middle, bending hard at the rim, so it reads as glass and not a smear.
    float dTag = sdTag(ps);
    ...
    // After the bend, so the cursor is refracted along with everything else.
    float toMouse = length(p - uMouse.xy);

    // A halo that softens the ring only where it is touched. Once per pixel rather than per plane, so it costs one length().
    float k = uK;
    if (uMouse.z > 0.001) {
    ...
    // The two nearest planes, tracked alongside the field so colour resolves without a second pass. Both are close in the goo, where the crossfade goes.
    float d0 = 1e6, d1 = 1e6;
    ...
    for (int i = 0; i < MAX_PLANES; i++) {
      ...
      vec2 q = p - uPos[i];
      // The cull that keeps 32 planes affordable. Scaled by the plane, since a swollen one reaches further than its resting size.
      ...
      // Clamped, or goo outside a plane samples the next atlas cell.
    ...
    for (int i = 0; i < MAX_LINKS; i++) {
      ...
      vec4 par = uLinkPar[i];
      // Negative radii lift the bridge's field clear so it fades out instead of bottoming out as a half-covered hairline.
    ...
    // A capillary wake off the cursor, dying over the same reach the softening uses, so a flick leaves a ripple that outlives the movement.
    if (uMelt.y > 0.001) {
      d += sin(toMouse * uMelt.z - uTime * uMelt.w)
         * uMelt.y * exp(-toMouse / max(uMelt.x, 1.0));
    }

    // Clamped, not floored: an unclamped fwidth across the cull step paints an outline along every cull boundary.
    float aa = clamp(fwidth(d), 0.5, 2.0);
    float alpha = 1.0 - smoothstep(-aa, aa, d);

    // The tag can overhang a card, and those pixels are its own.
    float taa = clamp(fwidth(dTag), 0.5, 2.0);
    float ta = tagOn > 0.001 ? 1.0 - smoothstep(-taa, taa, dTag) : 0.0;

    if (alpha <= 0.001 && ta <= 0.001) discard;

    // Even mix where the two nearest planes are equidistant. Art and dim both ride on it, so neither puts a seam down the goo.
    ...
    if (uTextured > 0.5) {
      ...
      // Uniform branch, keeping mip-selection derivatives defined. Scaling the offset by bend lands all three taps on one texel outside the lip.
      if (uFringe > 0.0) {
    ...
    // Cards standing aside are dimmed so the hovered one reads as lit.
    col *= mix(dm1, dm0, nearest);

    // Lift where the lip is steepest, so it catches light and is not only a warp.
    col += bend * uSheen;

    // --- the tag -------------------------------------------------------------
    if (ta > 0.001) {
        // Off the ring, the page shows through, so the label still has a ground.
        vec3 under = mix(uPage, col, alpha);
        vec3 glass = mix(under, vec3(1.0), uTagQ.x);

        // Signed, so the rim reads as thickness rather than a drawn-on outline.
        float band = clamp(1.0 + dTag / max(uTagP.z, 1.0), 0.0, 1.0);
        glass += band * band * uTagQ.y *
                 dot(tagNormal(ps), vec2(-0.7071, 0.7071));

        // Pure black or white per pixel, so a glyph crossing a light edge onto a dark one flips halfway across. A blend cannot: inverting mid grey returns mid grey.
    ...
    // Straight through: the atlas is NoColorSpace and this shader adds no output encoding, so decoding on read would darken everything.
